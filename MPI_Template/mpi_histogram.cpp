...
int main(int argc, char* argv[]) {
	...
	vector<int> total_bin_counts;
	int data_count;			  // cantidad de datos
	int local_data_count;	  // cantidad de datos por proceso
...
#  endif
	...
	local_data_count = data_count / cnt_proc;
	...
	total_bin_counts.resize(bin_count);

	/* Generate the data */
	gen_data(min_meas, max_meas, data, local_data_count);
...
#  endif
	...
	MPI_Reduce(&bin_counts, &total_bin_counts, bin_count, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
